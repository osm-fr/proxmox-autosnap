...
def get_zfs_volume(proxmox_fs: str) -> str:
    run = run_command(['pvesm', 'path', proxmox_fs])
    if not run['status']:
        raise SystemExit(run['message'])

    zfsvol = run['message'].strip().split(",")[0]
    if zfsvol[0] == "/":
        return zfsvol[1:]
    else:
        return zfsvol
...
def zfs_send(vmid: str, virtualization: list[str], zfs_send_to: str):
    ...
    localzfs  = get_zfs_volume(cfg["rootfs"])
    remotezfs = os.path.join(zfs_send_to, cfg["hostname"])
